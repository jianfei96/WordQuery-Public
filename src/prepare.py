...
def add_query_button(self):
    ...
    self.queryButton.setToolTip(
        shortcut(u"Query (shortcut: %s)" % my_shortcut))


def setup_browser_menu():

    def on_setup_menus(browser):
        menu = QMenu(_("WORD_QUERY"), browser.form.menubar)
        browser.form.menubar.addMenu(menu)
        action_queryselected = QAction(_("QUERY_SELECTED"), browser)
        ...
        action_options = QAction(_("OPTIONS"), browser)
...
def setup_context_menu():
    ...
    def on_setup_menus(web_view, menu):
        ...
        """
        wqmenu = menu.addMenu(_('WORD_QUERY'))
        action1 = wqmenu.addAction(_('QUERY_ALL_FIELDS'))
        action2 = wqmenu.addAction(_('QUERY_CURRENT_FIELD'))
        action3 = wqmenu.addAction(_('OPTIONS'))
